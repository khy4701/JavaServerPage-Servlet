...
$(function(){
	...
	$("#list").on("click","#btnDelete",function(){
			$.ajax({
				...
				// list의 btnDelete 버튼의 상대 위치로 replyNum 가져옴.
				data : {"replyNum": $(this).parent().prev().prev().prev().prev().text() , "boardNum": $("#boardNum").val()},
				dataType : "json",
				success: function(data){
					// view.jsp의 div 태그의 아이디를 읽어서 비워버림
					$("#list").empty();
			...
			<table border="1">
				<c:if test ="${requestScope.ReplyList != null }">
					<c:forEach var="reply" items="${ requestScope.ReplyList}">
   						<tr><td>${reply.num }</td><td>${reply.id}</td> <td>${reply.content}</td>
